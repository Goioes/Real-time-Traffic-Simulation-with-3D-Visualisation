...
public class TryMultiThreading : MonoBehaviour
{
    ...
    static void CreateChildThread()  // Main creates child thread
    {
        ...
        childThread.Start();
        Console.ReadKey();
        UnityEngine.Debug.Log(string.Format("{0} sleeps for 2 seconds..", mainThread.Name));
        Thread.Sleep(2000);
        UnityEngine.Debug.Log(string.Format("{0} awakes", mainThread.Name));
    }
    ...
    public static void CallToChildThread() // Child thread
    {
        ...
        string cmdStartSumo;
        cmdStartSumo = "sumo-gui -c " + SceneSwap.sceneSwap.scenarioPath + " --start --remote-port "
            + SceneSwap.sceneSwap.remotePort.ToString()
            + " --step-length " + SceneSwap.sceneSwap.stepLength.ToString(CultureInfo.InvariantCulture);
    ...
    {
        UnityEngine.Debug.Log(string.Format("Abort thread: {0}", childThread.Name));
        childThread.Abort();
        if (childThread.IsAlive)
        {
            UnityEngine.Debug.Log("ChildThread is dead");
        }
        ...
        {
            UnityEngine.Debug.Log(string.Format("Exception: {0}, {0}", e.Message, e.InnerException));
        }
    }
